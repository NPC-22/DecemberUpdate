...
class _ProgressReportPageState extends State<ProgressReportPage> {
  ...
  List<RatingData> myRatings = [];
  List<RatingData> otherRatings = [];
  ...
  void _processData(List<dynamic> data) {
    ...
    for (var item in data) {
      ...
      myRatings.add(RatingData(_splitLabel(item['title']), myRatingValue));
      otherRatings.add(
  ...
  void _generateChartData() {
    _barChartData = List.generate(myRatings.length, (index) {
      return BarChartGroupData(
        x: index,
        barRods: [
          BarChartRodData(
            toY: myRatings[index].rating,
            color: Colors.deepOrange,
            width: 8,
            borderRadius: BorderRadius.circular(4),
          ),
          BarChartRodData(
            toY: otherRatings[index].rating,
            color: Colors.blue,
            width: 8,
            borderRadius: BorderRadius.circular(4),
          ),
  ...
  void _nextPage() {
    setState(() {
      if (currentStartIndex + maxVisibleItems < myRatings.length) {
                        ...
                        bottomTitles: AxisTitles(
                          sideTitles: SideTitles(
                            ...
                            getTitlesWidget: (value, meta) {
                              ...
                              if (index >= 0 &&
                                  index < myRatings.length) {
                                return Text(
                                  myRatings[index].question,
                                  style: TextStyle(
                                    fontSize: 10,
                                  ),
                                );
